document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadBookings() {
        ...
        const searchTerm = document.querySelector('.header-search input').value.toLowerCase();
        ...
        const filteredBookings = bookings.filter(booking => {
            ...
            const searchableFields = [
                `#${String(bookings.indexOf(booking) + 1000).padStart(4, '0')}`, // Booking ID
                `${booking.firstName} ${booking.lastName}`, // Guest name
                booking.email,
                booking.phone,
                booking.checkIn,
                booking.checkOut,
                booking.status,
                '$150' // Amount (since it's fixed)
            ];
        ...
        filteredBookings.forEach((booking, index) => {
            ...
            viewBtn.addEventListener('click', () => viewBooking(booking));
            confirmBtn.addEventListener('click', () => updateStatus(booking, 'confirmed'));
            cancelBtn.addEventListener('click', () => updateStatus(booking, 'cancelled'));
    ...
    window.updateStatus = function(booking, newStatus) {
        const bookings = JSON.parse(localStorage.getItem('bookings')) || [];
        const index = bookings.findIndex(b => b.bookingDate === booking.bookingDate);
        if (index !== -1) {
    ...
    // Add this function to highlight search matches
